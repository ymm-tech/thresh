...
/**
 * {@code View} that displays a {@link SplashScreen} until a given {@link ThreshFlutterView} renders its
 * first frame.
 */
/* package */ final class ThreshFlutterSplashView extends FrameLayout {
    ...
    @NonNull
    private final ThreshFlutterView.FlutterEngineAttachmentListener flutterEngineAttachmentListener =
            new ThreshFlutterView.FlutterEngineAttachmentListener() {
                @Override
    ...
    public ThreshFlutterSplashView(@NonNull Context context) {
        this(context, null, 0);
    }

    public ThreshFlutterSplashView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ThreshFlutterSplashView(
    ...
    public void displayFlutterViewWithSplash(
            @NonNull ThreshFlutterView flutterView, @Nullable SplashScreen splashScreen) {
        ...
        if (splashScreen != null) {
            if (isSplashScreenNeededNow()) {
                ThreshLogger.v(TAG + "Showing splash screen UI.");
                ...
                ThreshLogger.v(TAG + "Showing an immediate splash transition to Flutter due to previously interrupted transition.");
                splashScreenView = splashScreen.createSplashView(getContext(), splashScreenState);
                addView(splashScreenView);
                transitionToFlutter();
            } else if (!flutterView.isAttachedToFlutterEngine()) {
                ThreshLogger.v(TAG + "FlutterView is not yet attached to a FlutterEngine. Showing nothing until a FlutterEngine is attached.");
    ...
    private void transitionToFlutter() {
        ...
        ThreshLogger.v(TAG + "Transitioning splash screen to a Flutter UI. Isolate: " + transitioningIsolateId);
